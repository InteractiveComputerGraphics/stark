...
When you add a cloth, a tet mesh, or a cable, its state is appended to one shared array of 3D positions velocities, etc.
This concatenated representation is results in a very clean internal structure and better parallelization and vectorization.
Each distinct object is identified by its `PointSetHandler`.
...
Usually, you will want to use a composition of different energies to model an object, such as inertia, triangle strain and discrete shell bending for cloth.
The [Presets](presets.md) layer wraps the most common combinations into single calls so you rarely need to compose energies by hand.
