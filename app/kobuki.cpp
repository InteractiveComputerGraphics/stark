...
int make_towel(stark::models::Simulation& sim)
{
	const double l = 1.35;
	const double w = 0.75;
	const double mass = 0.484;
	const double friction = 0.25;
	const int n_short_side = 30;

	std::vector<Eigen::Vector3d> vertices;
	std::vector<std::array<int, 3>> triangles;
	const std::array<int, 2> n = stark::utils::generate_triangular_grid(vertices, triangles, w, l, n_short_side);
	...
	sim.cloth.is_strain_limiting_active = true;
	sim.cloth.set_density(cloth_id, mass/(l*w));
