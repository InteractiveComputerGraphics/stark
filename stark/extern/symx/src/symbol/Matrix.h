...
namespace symx
{
	...
	public:
		...
		template<typename T>
		void set_values_row_major(const T& v);
	...
	template <typename T>
    inline void Matrix::set_values_row_major(const T &v)
    {
		for (int32_t i = 0; i < this->nrows; i++) {
			for (int32_t j = 0; j < this->ncols; j++) {
				this->vals[i * this->ncols + j].set_value(v(i, j));
			}
		}
    }
}