...
symx::Scalar symx::Scalar::ln() const
{
	const Scalar& u = (*this);
	if (u.is_one()) {
		return this->get_zero();
...
symx::Scalar symx::Scalar::log10() const
{
	const Scalar& u = (*this);
	if (u.is_one()) {
		return this->get_zero();
		...
		return Scalar(this->expressions->add_operation(ExprType::Log10, this->expr_id, -1), this->expressions);
	}
}
...
double symx::Scalar::eval() const
{
	return this->expressions->eval(this->expr_id);
}
std::string symx::Scalar::get_checksum() const
{
	...
	// Recursively add the expression to the hasher
    this->_get_checksum(hasher, visited);
...
void symx::Scalar::_get_checksum(picosha2::hash256_one_by_one& hasher, std::unordered_set<int32_t>& visited) const
{
    ...
    if (is_operation(this->expr.type) || this->expr.type == ExprType::Branch) {
        if (visited.find(this->expr.a) == visited.end()) {
            this->left()._get_checksum(hasher, visited);
        }
        if (this->has_right() && visited.find(this->expr.b) == visited.end()) {
            this->right()._get_checksum(hasher, visited);
        }
    }
...
symx::Scalar symx::ln(const Scalar& scalar)
{
	return scalar.ln();
}
symx::Scalar symx::log10(const Scalar& scalar)
{
	return scalar.log10();
}
