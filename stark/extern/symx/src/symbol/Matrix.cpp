...
symx::Matrix symx::Matrix::inv_sym() const
{
	assert(this->nrows == this->ncols && "symx error: cannot use symx::Matrix::inv() on non-square matrices.");
	Matrix m_inv = Matrix::zero({ this->nrows, this->ncols }, this->vals[0]);
	const Matrix& m = (*this);
	...
	if (this->nrows == 1) {
		...
		Scalar tmp0 = 1/(m(0, 0)*m(1, 1) - m(1, 0).powN(2));
		Scalar tmp1 = -m(1, 0)*tmp0;
		m_inv(0, 0) = m(1, 1)*tmp0;
		m_inv(0, 1) = tmp1;
		m_inv(1, 0) = tmp1;
		m_inv(1, 1) = m(0, 0)*tmp0;
		...
		Scalar tmp0 = m(1, 1)*m(2, 2);
		Scalar tmp1 = m(2, 1).powN(2);
		Scalar tmp2 = m(1, 0).powN(2);
		Scalar tmp3 = m(2, 0).powN(2);
		Scalar tmp4 = m(2, 0)*m(2, 1);
		Scalar tmp5 = 1/(-m(0, 0)*tmp0 + m(0, 0)*tmp1 - 2*m(1, 0)*tmp4 + m(1, 1)*tmp3 + m(2, 2)*tmp2);
		Scalar tmp6 = tmp5*(m(1, 0)*m(2, 2) - tmp4);
		Scalar tmp7 = tmp5*(-m(1, 0)*m(2, 1) + m(1, 1)*m(2, 0));
		Scalar tmp8 = tmp5*(m(0, 0)*m(2, 1) - m(1, 0)*m(2, 0));
		m_inv(0, 0) = tmp5*(-tmp0 + tmp1);
		m_inv(0, 1) = tmp6;
		m_inv(0, 2) = tmp7;
		m_inv(1, 0) = tmp6;
		m_inv(1, 1) = tmp5*(-m(0, 0)*m(2, 2) + tmp3);
		m_inv(1, 2) = tmp8;
		m_inv(2, 0) = tmp7;
		m_inv(2, 1) = tmp8;
		m_inv(2, 2) = tmp5*(-m(0, 0)*m(1, 1) + tmp2);
	}
	else if (this->nrows == 4) {
		Scalar tmp0 = m(3, 2).powN(2);
		Scalar tmp1 = m(1, 1)*tmp0;
		Scalar tmp2 = m(2, 1).powN(2);
		Scalar tmp3 = m(3, 3)*tmp2;
		Scalar tmp4 = m(3, 1).powN(2);
		Scalar tmp5 = m(2, 2)*tmp4;
		Scalar tmp6 = m(2, 2)*m(3, 3);
		Scalar tmp7 = m(1, 1)*tmp6;
		Scalar tmp8 = m(3, 1)*m(3, 2);
		Scalar tmp9 = 2*tmp8;
		Scalar tmp10 = m(1, 0).powN(2);
		Scalar tmp11 = m(3, 3)*tmp10;
		Scalar tmp12 = m(2, 0).powN(2);
		Scalar tmp13 = m(3, 3)*tmp12;
		Scalar tmp14 = m(3, 0).powN(2);
		Scalar tmp15 = m(2, 2)*tmp14;
		Scalar tmp16 = m(0, 0)*m(2, 1);
		Scalar tmp17 = m(2, 0)*m(3, 3);
		Scalar tmp18 = m(2, 1)*tmp17;
		Scalar tmp19 = 2*m(1, 0);
		Scalar tmp20 = m(2, 0)*tmp8;
		Scalar tmp21 = m(2, 1)*m(3, 0);
		Scalar tmp22 = m(3, 2)*tmp21;
		Scalar tmp23 = m(2, 2)*m(3, 0);
		Scalar tmp24 = m(3, 1)*tmp23;
		Scalar tmp25 = m(1, 1)*m(3, 2);
		Scalar tmp26 = m(3, 0)*tmp25;
		Scalar tmp27 = 2*m(2, 0);
		Scalar tmp28 = 1/(m(0, 0)*tmp1 + m(0, 0)*tmp3 + m(0, 0)*tmp5 - m(0, 0)*tmp7 + m(1, 1)*tmp13 + m(1, 1)*tmp15 + m(2, 2)*tmp11 + m(3, 1)*tmp21*tmp27 - tmp0*tmp10 - tmp12*tmp4 - tmp14*tmp2 - tmp16*tmp9 - tmp18*tmp19 + tmp19*tmp20 + tmp19*tmp22 - tmp19*tmp24 - tmp26*tmp27);
		Scalar tmp29 = tmp28*(-m(1, 0)*tmp0 + m(1, 0)*tmp6 - tmp18 + tmp20 + tmp22 - tmp24);
		Scalar tmp30 = m(1, 0)*m(2, 1);
		Scalar tmp31 = tmp28*(m(1, 0)*m(3, 1)*m(3, 2) + m(1, 1)*m(2, 0)*m(3, 3) - m(2, 0)*tmp4 + m(2, 1)*m(3, 0)*m(3, 1) - m(3, 3)*tmp30 - tmp26);
		Scalar tmp32 = m(2, 0)*m(3, 1);
		Scalar tmp33 = tmp28*(-m(1, 0)*m(2, 2)*m(3, 1) + m(1, 1)*tmp23 - m(2, 0)*tmp25 + m(2, 1)*tmp32 - m(3, 0)*tmp2 + m(3, 2)*tmp30);
		Scalar tmp34 = m(2, 0)*m(3, 0);
		Scalar tmp35 = tmp28*(-m(0, 0)*tmp8 + m(1, 0)*m(3, 0)*m(3, 2) - m(1, 0)*tmp17 - m(2, 1)*tmp14 + m(3, 0)*tmp32 + m(3, 3)*tmp16);
		Scalar tmp36 = tmp28*(m(0, 0)*m(2, 2)*m(3, 1) + m(1, 0)*m(2, 0)*m(3, 2) - m(1, 0)*tmp23 + m(2, 0)*m(2, 1)*m(3, 0) - m(3, 1)*tmp12 - m(3, 2)*tmp16);
		Scalar tmp37 = m(0, 0)*m(1, 1);
		Scalar tmp38 = tmp28*(m(0, 0)*tmp25 + m(1, 0)*tmp21 + m(1, 0)*tmp32 - m(1, 1)*tmp34 - m(3, 1)*tmp16 - m(3, 2)*tmp10);
		m_inv(0, 0) = tmp28*(-m(2, 1)*tmp9 + tmp1 + tmp3 + tmp5 - tmp7);
		m_inv(0, 1) = tmp29;
		m_inv(0, 2) = tmp31;
		m_inv(0, 3) = tmp33;
		m_inv(1, 0) = tmp29;
		m_inv(1, 1) = tmp28*(m(0, 0)*tmp0 - m(0, 0)*tmp6 - 2*m(3, 2)*tmp34 + tmp13 + tmp15);
		m_inv(1, 2) = tmp35;
		m_inv(1, 3) = tmp36;
		m_inv(2, 0) = tmp31;
		m_inv(2, 1) = tmp35;
		m_inv(2, 2) = tmp28*(m(0, 0)*tmp4 + m(1, 1)*tmp14 - m(3, 0)*m(3, 1)*tmp19 - m(3, 3)*tmp37 + tmp11);
		m_inv(2, 3) = tmp38;
		m_inv(3, 0) = tmp33;
		m_inv(3, 1) = tmp36;
		m_inv(3, 2) = tmp38;
		m_inv(3, 3) = tmp28*(m(0, 0)*tmp2 + m(1, 1)*tmp12 + m(2, 2)*tmp10 - m(2, 2)*tmp37 - tmp27*tmp30);	
	}
	else {
		std::cout << "symx error: Matrix inverse larger than 4x4 is not supported." << std::endl;
		exit(-1);
...
symx::Matrix symx::Matrix::operator+(const Matrix& other) const
{
	...
	std::vector<Scalar> result_vals;
	for (int i = 0; i < this->nrows; i++) {
		for (int j = 0; j < this->ncols; j++) {
			result_vals.push_back((*this)(i, j) + other(i, j));
		}
	}
	return Matrix(result_vals, { this->nrows, this->ncols });
}
symx::Matrix symx::Matrix::operator-(const Matrix& other) const
{
	this->_assert_consistent_size(other);
...
symx::Matrix symx::Matrix::operator*(const Scalar& scalar) const
{
	...
	for (int i = 0; i < (int)this->vals.size(); i++) {
		result_vals.push_back(this->vals[i]*scalar);
	}
	return Matrix(result_vals, { this->nrows, this->ncols });
}
symx::Vector symx::Matrix::operator*(const Vector& vec) const
{
	...
	for (int i = 0; i < this->nrows; i++) {
		...
		result_vals.push_back(sum);
	}
	return Vector(result_vals);
}
symx::Matrix symx::Matrix::operator*(const Matrix& other) const
{
	...
	return Matrix(result_vals, { this->nrows, other.ncols });
}
symx::Matrix symx::Matrix::operator/(double val) const
{
	return (*this)/this->vals[0].make_constant(val);
