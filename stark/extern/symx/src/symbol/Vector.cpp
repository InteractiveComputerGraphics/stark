...
symx::Vector symx::Vector::segment(const int32_t begin, const int32_t count) const
{
	assert(begin >= 0 && begin + count <= this->n);
...
void symx::Vector::set_segment(const int32_t begin, const int32_t count, const Vector& other)
{
	assert(begin >= 0 && begin + count <= this->n);
...
symx::Scalar symx::Vector::dot(const Vector& other) const
{
	...
	assert(size == other.size() && "symx error: Dot product between symx::Vector of different sizes");
...
symx::Scalar symx::Vector::stable_norm(const Scalar& eps) const
{
	return (this->squared_norm() + eps).sqrt() - eps.sqrt();
}
symx::Scalar symx::Vector::stable_norm(double eps) const
{
    return this->stable_norm(this->vals[0].make_constant(eps));
}
