...
bool stark::EnergyFrictionalContact::_should_continue_execution(core::Stark& stark)
{
	...
	if (this->contact_stiffness > this->global_params.max_contact_stiffness) {
		stark.logger.set("success", 0);
		stark.console.print(fmt::format("Contact stiffness exceeded maximum value of {:.1e}. ", this->global_params.max_contact_stiffness), core::ConsoleVerbosity::Frames);
		return false;
	}
	else {
		return true;
