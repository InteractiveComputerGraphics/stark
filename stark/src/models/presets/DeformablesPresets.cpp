...
{
	auto [vertices, segments] = utils::generate_segment_line(begin, end, n_segments);
...
{
	auto [vertices, triangles] = utils::generate_triangle_grid({ 0.0, 0.0 }, dim, subdivisions);
...
{
	auto [surface_triangles, tri_to_tet_map] = stark::utils::extract_surface(vertices, tets);
...
{
	auto [vertices, tets] = utils::generate_tet_grid({ 0.0, 0.0, 0.0 }, dim, subdivisions);
